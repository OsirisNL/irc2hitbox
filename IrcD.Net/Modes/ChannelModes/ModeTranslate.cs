    ...
    class ModeTranslate : ChannelMode
    {
        ...
        public override bool HandleEvent(CommandBase command, ChannelInfo channel, UserInfo user, List<string> args)
        {
            ...
            if (command is PrivateMessage || command is Notice)
            {
                ...
                translateDelegate.BeginInvoke(args[1], channel.Users.Select(u => u.Languages.First()).Distinct(), TranslateCallBack, IrcD.Utils.Tuple.Create(channel, user, command));
        ...
        {
            var state = (IrcD.Utils.Tuple<ChannelInfo, UserInfo, CommandBase>)asyncResult.AsyncState;
            ...
            {
                IrcD.Utils.Tuple<string, string, string> res;
                string message;
